...
class Cluster:
    def __init__(self,hits,rebin,img_fr,img_fr_zs,geometry,options, debug=False,fullinfo=False,clID=0):
        self.hits = hits
        self.rebin = rebin
        self.debug = debug
    ...
    def calcProfiles(self,name='prof',plot=None):
        ...
        #xedg = np.unique([int(h[0]) for h in rot_hits]).tolist()
        #yedg = np.unique([int(h[1]) for h in rot_hits]).tolist()
        ...
        #if not self.options.find_peak:
        self.shapes['long_width'] = self.widths['long']
        self.shapes['lat_width'] = self.widths['lat']
        self.shapes['long_fullrms']          = self.profiles['long'].GetRMS()
        self.shapes['lat_fullrms']          = self.profiles['lat'].GetRMS()
