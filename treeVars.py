...
class AutoFillTreeProducer:
    def __init__(self,tree,eventContent, options):
        self.outTree = tree
    ...
    def createClusterVariables(self, name='track'):
        ...
        #if self.options.empty_variables: self.outTree.branch('{name}_corrintegral'.format(name=name), 'F', lenVar=sizeStr, title="density-corrected integral of the cluster (LEMON-specific calibration)")
        ...
        #    if self.options.empty_variables:
        ...
        #if self.options.empty_variables: self.outTree.branch('{name}_pearson'.format(name=name),      'F', lenVar=sizeStr, title="Pearson coefficient of the cluster")
        
        #if self.options.gaus_fit:
    ...
    def fillTimeVariables(self,variables, DBSCAN, lp):
        self.outTree.fillBranch('t_DBSCAN',DBSCAN)
        self.outTree.fillBranch('t_variables',variables)
    ...
    def fillClusterVariables(self,clusters,name='track'):
        ...
        #if self.options.empty_variables: self.outTree.fillBranch('{name}_corrintegral'.format(name=name), [cl.corr_integral() for cl in clusters])
        ...
        #if self.options.empty_variables: self.outTree.fillBranch('{name}_pearson'.format(name=name),   [cl.getPearson() for cl in clusters])
