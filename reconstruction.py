...
class analysis:
    ...
    def beginJob(self,outfname):
        ...
        self.autotree = AutoFillTreeProducer(self.outTree,self.eventContentParams, self.options)
...
#           self.outTree.branch("MC_track_len","F")
    ...
    def calcPedestal(self,options,alternativeRebin=-1):
        ...
        if options.rawdata_tier == 'root':
            ...
            postfix = 'root'
            tf = sw.swift_read_root_file("%s/%s%05d.%s" % (tmpdir,prefix,int(options.pedrun),postfix))
            keys = tf.keys()
        else:
            ...
            for iobj,key in enumerate(keys):
                ...
                if self.options.camera_mode:
                    if camera==True:
                        ...
                        snakes, t_DBSCAN, t_variables, lp_len = snprod.run()
                        ...
                        self.autotree.fillTimeVariables(t_variables, t_DBSCAN, lp_len)
