...
class SnakesFactory:
    ...
    def getClusters(self,plot=False):
        ...
        for k in unique_labels:
            ...
            if k>-1 and len(x)>1:
                cl = Cluster(xy,self.rebin,image_fr_vignetted,image_fr_zs_vignetted,self.options.geometry,self.options, debug=False,fullinfo=self.options.scfullinfo,clID=k)
                cl.iteration = 0
    ...
    def calcProfiles(self,clusters,plot=False):
        #print(clusters)
        #np.save("rot_hits_im.npy", clusters)
        for k,cl in enumerate(clusters):
...
class SnakesProducer:
    ...
    def run(self):
        ...
        if self.algo=='DBSCAN':
            snakes, lp_len = snfac.getClusters(plot=self.plotpy)
        ...
        print(f"  1.1 DBSCAN in {t1 - t0:0.4f} seconds")
        t_DBSCAN = t1-t0
        ...
        return snakes, t_DBSCAN, t_variabili, lp_len